...
/** @file
 *
 * This file shows how to create a mesh dimmable light which allows color temperature adjustments.
 *
 */
...
{
    WICED_BT_MESH_MODEL_LIGHT_CTL_TEMPERATURE_SERVER,
};
...
wiced_bt_mesh_core_config_element_t mesh_elements[] =
{
    ...
    },
    {
        ...
        .default_transition_time = MESH_DEFAULT_TRANSITION_TIME_IN_MS,  // Default transition time for models of the element in milliseconds
        .onpowerup_state = WICED_BT_MESH_ON_POWER_UP_STATE_RESTORE,     // Default element behavior on power up
        .default_level = 0x3000,                                        // Default value of the variable controlled on this element (for example power, lightness, temperature, hue...)
        .range_min = WICED_BT_MESH_LIGHT_CTL_TEMPERATURE_MIN,           // Minimum value of the Color Temperature
        .range_max = WICED_BT_MESH_LIGHT_CTL_TEMPERATURE_MAX,           // Maximum value of the Color Temperature
        .move_rollover = 0,                                             // If true when level gets to range_max during move operation, it switches to min, otherwise move stops.
        .properties_num = 0,                                            // Number of properties in the array models
        .properties = NULL,                                             // Array of properties in the element.
        .sensors_num = 0,                                               // Number of sensors in the sensor array
...
void mesh_app_init(wiced_bool_t is_provisioned)
{
    ...
    wiced_bt_cfg_settings.device_name = (uint8_t *)"CTL Light";
...
/*
 * Process event received from the models library for mesh ctl lighting.
 */
void mesh_app_message_handler(uint8_t element_idx, uint16_t event, void *p_data)
{
    WICED_BT_TRACE("light ctl srv msg:%d\n", event);
...
uint32_t mesh_app_proc_rx_cmd(uint16_t opcode, uint8_t *p_data, uint32_t length)
{
    ...
    case HCI_CONTROL_MESH_COMMAND_LIGHT_CTL_SET:
        ...
        mesh_light_ctl_server_status_changed(element_idx, p_data, length);
        break;
...
/*
 * Send Light CTL State changed
 */
void mesh_light_ctl_server_status_changed(uint8_t element_idx, uint8_t *p_data, uint32_t length)
{
    wiced_bt_mesh_light_ctl_status_data_t data;

    STREAM_TO_UINT16(data.present.lightness, p_data);
    STREAM_TO_UINT16(data.present.temperature, p_data);
    STREAM_TO_UINT16(data.present.delta_uv, p_data);
    data.remaining_time = 0;

    wiced_bt_mesh_model_light_ctl_server_state_changed(element_idx, &data);
}
